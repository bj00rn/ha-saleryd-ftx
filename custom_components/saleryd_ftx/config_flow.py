    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            valid = await self._test_connection(
                user_input[CONF_WEBSOCKET_URL],
            )
            if valid:
                return self.async_create_entry(
                    title=user_input[CONF_WEBSOCKET_URL], data=user_input
                )
            else:
        ...
        user_input[CONF_WEBSOCKET_URL] = "ws://192.168.1.151:3001"
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        CONF_WEBSOCKET_URL,
                        default=user_input[CONF_WEBSOCKET_URL],
                    ): str,
                }
            ),
            errors=self._errors,
...
class SalerydLokeOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def _update_options(self):
        ...
        return self.async_create_entry(
            title=self.config_entry.data.get(CONF_WEBSOCKET_URL), data=self.options
        )
